...
def fwhm2s(fwhm_amin):
    return fwhm_amin * np.pi / (2.355*180*60)
...
def get_component_spectra(info, lmax):
    ...
    _,dls_cmbl_ee,dls_cmbl_bb,_=read_camb("./examples/data/camb_lens_nobb.dat", lmax)
    _,dls_cmbt_ee,dls_cmbt_bb,_=read_camb("./examples/data/camb_lens_r1.dat", lmax)
    return (dls_sync_ee, dls_sync_bb,
            dls_dust_ee, dls_dust_bb,
            dls_cmbl_ee, Alens*dls_cmbl_bb+r*(dls_cmbt_bb-dls_cmbl_bb))

def get_convolved_seds(info, names, bpss):
    beta_sync = info['fg'].get('beta_sync', -3.1)
    nu0_sync = 23.
