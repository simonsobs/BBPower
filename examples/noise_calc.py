...
def Simons_Observatory_V3_SA_beams(ell):
    SA_beams = (
        Simons_Observatory_V3_SA_beam_FWHM()
        / np.sqrt(8.0 * np.log(2))
        / 60.0
        * np.pi
        / 180.0
    ...
    return [np.exp(-0.5 * ell * (ell + 1) * sig**2.0) for sig in SA_beams]
...
def Simons_Observatory_V3_SA_noise(
    ...
    if include_beam:
        ...
        N_ell_P_39 /= SA_beams[1] ** 2.0
        N_ell_P_93 /= SA_beams[2] ** 2.0
        N_ell_P_145 /= SA_beams[3] ** 2.0
        N_ell_P_225 /= SA_beams[4] ** 2.0
        N_ell_P_280 /= SA_beams[5] ** 2.0
