...
"""
Script to combine simulations into observed sky splits. Will save a
signal-only file called "SO_SAT_maps_sky_signal.fits". Will save 4 obs splits
"SO_SAT_obs_map_split_[id_split]of4.fits".
All maps are saved as healpix fits files with 18 maps, corresponding to TQU
for 6 frequencies. Use np.reshape(6, 3, -1) to get [frequency, TQU, npix]. 
DO NOT USE T.

To parse:
* simsdir, e.g. '/global/cscratch1/sd/kwolz/BBPower/sims/nside512/full/r0_inhom/gaussian/baseline/optimistic'  # noqa
* seed:    4-digit zero-padded number between 0000 and 0499.
           Choose deterministically from cmb, fg and noise sims  # noqa
"""


def get_sky_signals(nrs, r001, Alens05, foreground, nside, freq_labels, fdir,
                    fgnames):
    """
    """
    ...
    for k, fl in enumerate(freq_labels):
        if r001 and not Alens05:  # r=0.001
            fname = f'{fdir}/CMB_r_Alens_20211108/r001_Alens1/cmb/{nrs}/'
            fname += f'SO_SAT_{fl}_cmb_{nrs}_CMB_r001_Alens1_20211108.fits'
        elif Alens05 and not r001:  # Alens=0.5
            fname = f'{fdir}/CMB_r_Alens_20211108/r0_Alens05/cmb/{nrs}/'
            fname += f'SO_SAT_{fl}_cmb_{nrs}_CMB_r0_Alens05_20211108.fits'
        elif Alens05 and r001:  # r=0.001 and Alens=0.5
            fname = f'{fdir}/CMB_r_Alens_20211108/r001_Alens05/cmb/{nrs}/'
            fname += f'SO_SAT_{fl}_cmb_{nrs}_CMB_r001_Alens05_20220516.fits'
        ...
        for fgn in fgnames:
            if foreground == 'gaussian':
                ...
                fname += f'SO_SAT_{fl}_{fgn}_{nrs}_gaussian_20201207.fits'
            elif foreground == 'd10s5':  # d10s5
                fname = f'{fdir}/FG_20220516/d10s5/foregrounds/{fgn}/'
                fname += f'SO_SAT_{fl}_{fgn}_d10s5_20220516.fits'
            else:  # d0s0, d1s1, dmsm
...
def get_noise(nrs, sp, sensitivity_mode, one_over_f, inhom, fdir_inhom, nside,
              freq_labels, mask=None):
    """
    """
            ...
            if sensitivity_mode == 2:
                if one_over_f == 1:
                    if mask == 'full-opt-al1':  # new lensing-optimized mask
                        fname = '/global/cfs/cdirs/sobs/users/emilie_h/'
                        fname += f'Noise_sims_May2023/goal_optimistic/{nrs}/'
                        fname += f'SO_SAT_{fl}_noise_split_{sp+1}of4_{nrs}_'
                        fname += 'goal_optimistic.fits'
                    else:  # new goal white noise level
                        fname = '/global/cfs/cdirs/sobs/users/krach/BBSims/'
                        fname += f'NOISE_20230531/goal_optimistic/{nrs}/'
                        fname += f'SO_SAT_{fl}_noise_split_{sp+1}of4_{nrs}_'
                        fname += 'goal_optimistic_20230531.fits'
                elif one_over_f == 0:
                    fname = f'{fdir_inhom}/goal_pessimistic/{nrs}/'
                    fname += f'SO_SAT_{fl}_noise_split_{sp+1}of4_{nrs}_'
                    fname += 'goal_pessimistic_20210727.fits'
            elif sensitivity_mode == 1:
                if one_over_f == 1:
                    fname = f'{fdir_inhom}/baseline_optimistic/{nrs}/'
                    fname += f'SO_SAT_{fl}_noise_split_{sp+1}of4_{nrs}'
                    fname += '_baseline_optimistic_20210727.fits'
                elif one_over_f == 0:
...
def run(sdir, nrs, r001, Alens05, sensitivity_mode, one_over_f, foreground,
        inhom, freq_labels, fdir, fdir_inhom, fgnames, overwrite=False,
        nside=512):
    """
    ...
    fdir = '/global/cfs/cdirs/sobs/users/krach/BBSims'  # noise map directory
...
def main(simsdir, seed):
    """
    To parse:
    * simsdir, e.g. '/pscratch/sd/k/kwolz/BBPower/sims/nside512/full/r0_inhom/gaussian/baseline/optimistic'  # noqa
    * seed:    4-digit zero-padded number between 0 and 499
    ...
    band = False
    sky = 'full'
    if ('north' in simsdir.split('/')[-5]) or ('north' in simsdir.split('/')[-6]):  # noqa
        sky = 'north'
    if ('south' in simsdir.split('/')[-5]) or ('south' in simsdir.split('/')[-6]):  # noqa
        sky = 'south'
    if ('full-opt-al1' in simsdir.split('/')[-5]) or ('full-opt-al1' in simsdir.split('/')[-6]):  # noqa
        sky = 'full-opt-al1'  # Optimized mask (AL=1 case)
    if ('inhom' in simsdir.split('/')[-4]) or ('inhom' in simsdir.split('/')[-5]):  # noqa
        inhom = True
    if ('band' in simsdir.split('/')[-4]) or ('band' in simsdir.split('/')[-5]):  # noqa
        band = True
    r001 = False
    if ('r001' in simsdir.split('/')[-4]) or ('r001' in simsdir.split('/')[-5]):  # noqa
        r001 = True
    Alens05 = False
