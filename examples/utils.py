...
def read_camb(fname, lmax):
    ...
    ell, dtt, dee, dbb, dte = np.loadtxt(fname, unpack=True)
    ell = ell.astype(int)
    msk = ell <= lmax
    ell = ell[msk]
...
def get_component_spectra(lmax, so_forecast=False):
    if so_forecast:  # foreground parameters from 2302.04276
        A_sync_BB = 1.6
        alpha_sync_EE = -0.7
        alpha_sync_BB = -0.93
        A_dust_BB = 28.
        alpha_dust_EE = -0.32
        alpha_dust_BB = -0.16
    else:  # foreground parameters from 2011.02449
        A_sync_BB = 2.0
        alpha_sync_EE = -0.6
        alpha_sync_BB = -0.4
        A_dust_BB = 5.
    ...
    _, dls_cmb_ee, dls_cmb_bb, _ = read_camb(
...
def get_convolved_seds(names, bpss, so_forecast=False):
    ...
    for ib, n in enumerate(names):
        b = bpss[n]
        seds[0, ib] = b.convolve_sed(lambda nu: comp_sed(nu, None, None, None, 'cmb'))  # noqa
        seds[1, ib] = b.convolve_sed(lambda nu: comp_sed(nu, nu0_sync, beta_sync, None, 'sync'))  # noqa
        seds[2, ib] = b.convolve_sed(lambda nu: comp_sed(nu, nu0_dust, beta_dust, temp_dust, 'dust'))  # noqa
    return seds
