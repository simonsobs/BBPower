...
class BBPowerSpecter(PipelineStage):
    ...
    def run(self):
        ...
        # if txt file is not existent
        tname = self.get_output('cells_all_sims')
        ...
        # Compile list of cells from txt (added SA)
        ccls_all = []
        with open(tname, 'r') as t:
            for ttname in t:
                ccls_all.append(ttname)
        for isim, d in enumerate(sims):
            # changed SA
            ffname = (ccls_all[isim]).strip() #stripping white spaces 
            if os.path.isfile(ffname):
                print("found " + ffname) #don't re-compute
            else:
