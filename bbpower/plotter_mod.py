...
class BBPlotter_mod(PipelineStage):
    ...
    def add_contours(self):
        ...
        with self.doc:
            ...
            truth={'A_lens':1.,
                   ...
                   'amp_d_bb':44.64, #45.1,#44.64,
                   'c_d_bb':0.14,#0.
                   'beta_s':-3.,
                   'alpha_s_bb':-0.78, #-0.99, #-0.78,
                   #'c_s_bb':0.17, #0.
                   'amp_s_bb':1.07}#1.10}#1.07}

            # Select only parameters for which we have labels
            names_common=list(set(list(self.chain['names'])) & labdir.keys())
            ...
            #print('Chain shape: ',self.chain['chain'].shape)
            ...
            for i,n in enumerate(names_common):
                if n in truth.keys():
                    ...
                    for j in range(i+1,npar):
                        if names_common[j] in truth.keys():
    ...
    def read_inputs(self):
        ...
        if self.config['plot_coadded_total']:
            ...
            self.s_cd_t_cov=sacc.Sacc.load_fits(self.get_input('cells_coadded_total_cov'))
        if self.config['plot_noise']:
            self.s_cd_n=sacc.Sacc.load_fits(self.get_input('cells_noise'))
            self.s_cd_n_cov=sacc.Sacc.load_fits(self.get_input('cells_noise_cov'))
        if self.config['plot_nulls']:
