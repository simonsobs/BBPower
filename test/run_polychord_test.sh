...
do
   ...
   python examples/generate_SO_maps.py \
     --output-dir test/test_out/s${seed} \
     --seed ${seed} \
     --nside 64
done
...
python -m bbpower BBPowerSpecter \
  ...
  --masks_apodized=./examples/data/maps/norm_nHits_SA_35FOV.fits \
  ...
  --cells_all_splits=./test/test_out/cells_all_splits.fits \
  --cells_all_sims=./test/test_out/cells_all_sims.txt \
  --mcm=./test/test_out/mcm.dum \
  --config=./test/test_config_polychord.yml
...
python -m bbpower BBPowerSummarizer \
  ...
  --cells_all_splits=./test/test_out/cells_all_splits.fits \
  --cells_all_sims=./test/test_out/cells_all_sims.txt \
  --cells_coadded_total=./test/test_out/cells_coadded_total.fits \
  --cells_coadded=./test/test_out/cells_coadded.fits \
  --cells_noise=./test/test_out/cells_noise.fits \
  --cells_null=./test/test_out/cells_null.fits \
  --config=./test/test_config_polychord.yml
...
python -m bbpower BBCompSep \
  ...
  --cells_coadded_cov=./test/test_out/cells_coadded.fits \
